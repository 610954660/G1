...
local StrideRankGJGameView,Super = class("StrideRankGJGameView", Window)

function StrideRankGJGameView:ctor()
	--LuaLog("StrideRankGJGameView ctor")
	self._packName = "StrideServer"
	self._compName = "StrideRankGJGameView"
...
function StrideRankGJGameView:_initEvent( )
	
end

function StrideRankGJGameView:_initVM( )
	local viewNode = self.view
	---Do not modify following code--------
	--{autoFields}:StrideServer.StrideRankGJGameView
	self.bar0_1 = viewNode:getChildAutoType('$bar0_1')--GProgressBar
	...
	self.heroCell0_1 = viewNode:getChildAutoType('$heroCell0_1')--Component8
		self.heroCell0_1.btn_video = viewNode:getChildAutoType('$heroCell0_1/$btn_video')--GButton
		self.heroCell0_1.heroCell = viewNode:getChildAutoType('$heroCell0_1/heroCell')--GButton
		self.heroCell0_1.img_shouzhi = viewNode:getChildAutoType('$heroCell0_1/img_shouzhi')--GImage
		self.heroCell0_1.img_shouzhiani = viewNode:getChildAutoType('$heroCell0_1/img_shouzhiani')--GLoader
		self.heroCell0_1.txt_num = viewNode:getChildAutoType('$heroCell0_1/txt_num')--GRichTextField
		self.heroCell0_1.txt_playername = viewNode:getChildAutoType('$heroCell0_1/txt_playername')--GTextField
	...
	self.img_ani0_1 = viewNode:getChildAutoType('img_ani0_1')--GLoader
	...
	self.oneBtn = viewNode:getChildAutoType('oneBtn')--GButton
	self.statusTitle = viewNode:getChildAutoType('statusTitle')--GTextField
	self.twoBtn = viewNode:getChildAutoType('twoBtn')--GButton
	self.txt_countdown = viewNode:getChildAutoType('txt_countdown')--GTextField
	--{autoFieldsEnd}:StrideServer.StrideRankGJGameView
	--Do not modify above code-------------
end

function StrideRankGJGameView:_initListener( )
	...
	self.oneBtn:addClickListener(function()
		StrideServerModel:setGJIndex(1)
		self.c1:setSelectedIndex(0)
		self:upWorldViewInfo()
	end)
	

	self.twoBtn:addClickListener(function()
		StrideServerModel:setGJIndex(2)
	...
	self.comboBox:addEventListener(FUIEventType.Changed,function(data)
		...
		local championCurArr = StrideServerModel:getCurChampionCurArr()
		if championCurArr.zid~=tonumber(value) then
			StrideServerModel:setChamptionCurZid(value)
			StrideServerModel:reqGetChamptionPanelInfo(value)
		end
   end)

end

function StrideRankGJGameView:_initUI( )
	self:_initVM()
	self:_initListener()
	local championCurArr = StrideServerModel:getCurChampionCurArr()
	if not championCurArr then
		StrideServerModel:reqGetChamptionPanelInfo(1)
	else
		StrideServerModel:reqGetChamptionPanelInfo(championCurArr.zid)
	end
	self.c1:setSelectedIndex(StrideServerModel:getGJIndex()-1)
end


function StrideRankGJGameView:_refresh( )
	local championCurArr = StrideServerModel:getCurChampionCurArr()
	if not championCurArr then
		StrideServerModel:reqGetChamptionPanelInfo(1)
	else
...
--更新整个页面信息
function StrideRankGJGameView:upWorldViewInfo()
	self:showBtn()
	--显示剩余时间
	...
	elseif bigStage >= 3 then
		...
		if bigStage == 3 and StrideServerModel:getGJIndex() ==1 then
			self:showCountDown(smallTime)
		elseif bigStage == 4 and StrideServerModel:getGJIndex() ==2 then
				self:showCountDown(smallTime)
		elseif bigStage == 4 and StrideServerModel:getGJIndex() ==1 then
			self.statusTitle:setVisible(false)
			self.txt_countdown:setText(Desc.DreamMasterPvp_allend)
		elseif bigStage == 3 and StrideServerModel:getGJIndex() ==2 then
			self.statusTitle:setVisible(false)
			self.txt_countdown:setText("未开赛")
		else
			self.statusTitle:setVisible(true)
...
--数据刷新
function StrideRankGJGameView:update_stride_champtionGJPvp()
	self:upWorldViewInfo() --刷新UI
end


--小组赛切换按钮
function StrideRankGJGameView:showBtn()
	if StrideServerModel:getGJIndex() ~= 1 then --第一轮才有按钮
       return
	end
	local championCurArr = StrideServerModel:getCurChampionCurArr()
	if #self.addMap == 0 then
		local index = math.min(6, math.max(1, championCurArr.sgid))
		self.addTag = index
		self.curGroupId = math.min(8, math.max(1, championCurArr.sgid))
	...
	for i = 1, 5, 1 do
		...
		btnItem:addClickListener(
			function(context)
				...
				self.touchBtn = btnItem
				self.curGroupId = num
				local  zoomID = StrideServerModel:getCurChampionCurArr().zid
				StrideServerModel:setChamptionCurArr(zoomID,num)
				self:showView() --更新界面
			end,
			100
		)
...
--往左 往右按钮事件
function StrideRankGJGameView:getBtnNum(type) --0减1加
	self.addMap = {}
	local flag = type == 0 and -1 or 1
	local tag = self.addTag + 5 * flag
	tag = math.min(4, math.max(1, tag))
	for i = 0, 4 do
...
--更新单个位置
function StrideRankGJGameView:itemCellUpdate(posKey,dataInfo,type,firstVal)
	...
	elseif type ==2 then
		...
		if battleInfo then
			...
			playerInfo = StrideServerModel:getChampionPlayInfo(pid)
		end
	end
...
--显示玩家显示样式
function StrideRankGJGameView:showView()
	local allData = StrideServerModel:getChampionData()
	if not allData then
       return
	end
	local curChampionArr = StrideServerModel:getCurChampionCurArr()
	
	local  bigStage,battleStage = StrideServerModel:getTwoStage()
	if StrideServerModel:getGJIndex() ==1  then
		if bigStage > 3 then --第一轮 已经超
			self.gameType2:setText("64晋8")
		elseif bigStage< 3 then
			self.gameType2:setText("64晋8")
		else
	...
	local data = {}
	if StrideServerModel:getGJIndex() ==1  then
		data =  allData.battleInfo[curChampionArr.sgid]
	else
		data =  allData.battleInfo2[curChampionArr.sgid2]
	end
	
	local first = 64
	if  StrideServerModel:getGJIndex() ==1 then --晋级赛已经过了
		first = 64
	elseif StrideServerModel:getGJIndex() ==2 then
		first = 8
	end
	--具体的显示位置
	local ItemPosNum = StrideServerModel.ItemPosGJ
	for i = 1, 8, 1 do
		local posKey = ItemPosNum[i] --左右两边8个
	    local tempData  = false
		if  data and data.playerList then
			tempData = data.playerList[i]
		end
		self:itemCellUpdate(posKey,tempData,1,first)
	end
	...
	for i = 9,#ItemPosNum,1 do
		...
		elseif  tonumber(arr[1])  == 1 then
			pos = pos + 10
		elseif  tonumber(arr[1])  == 0 then
			pos = 31
		end 
		local battleInfo = false
		if data and data.battleNodeList then
...
--显示线
function StrideRankGJGameView:showLine()
	local lineNum = StrideServerModel.LinePosGJ
	for lineId = 1, #lineNum, 1 do
		...
		local isShowAnim = StrideServerModel:getWorldChallengeXiantiao(tiaoKey,2) --获取需要显示特效的线组
		...
		if isShowAnim then
			if not self.xiantiaoAnima[tiaoKey] then
				print(1,"创建")
				self.xiantiaoAnima[tiaoKey] = self:getXiantiaoAnim(tiaoKey, img_ani,2)
			end
		else
		...
		barItem:setMin(0)
		barItem:setMax(100)
		local isLine = StrideServerModel:getLineBarState(tiaoKey,2)
		if isLine == 1 then
...
--获取线条
function StrideRankGJGameView:getXiantiaoAnim(lineId, barImg,type)
...
--显示倒计时
function StrideRankGJGameView:showCountDown(countdown)
	if self.calltimer then
		TimeLib.clearCountDown(self.calltimer)
		self.calltimer = false
	end
...
function StrideRankGJGameView:_exit()
	if self.calltimer then
		TimeLib.clearCountDown(self.calltimer)
		self.calltimer = false
	end
end
return StrideRankGJGameView