...
local ActShrineShopView = class("ActShrineShopView",Window)

function ActShrineShopView:ctor()
    self._packName = "ActShrineShop"
    self._compName = "ActShrineShopView"
...
function ActShrineShopView:_initUI()
    ...
    local viewData = ActivityModel:getActityByType(GameDef.ActivityType.ShrinePrayShop)
    printTable(1,viewData.showContent.titleBanner)
    self.banner:setURL(PathConfiger.getActivityBannerTitleStr(viewData.showContent.titleBanner))
    
end

function ActShrineShopView:_initEvent()
    self:ActShrineShop_refreshPanal()
end

function ActShrineShopView:ActShrineShop_refreshPanal()
    self:refreshPanal()
end

function ActShrineShopView:refreshPanal()
    self:setGiftList()
    self:updateCountTimer()
end

-- 设置礼包列表
function ActShrineShopView:setGiftList()
    local giftData = ActShrineShopModel:getShopData()
    self.list_gift:setItemRenderer(function(idx,obj)
        ...
        if data.price == 0 then
            ...
            RedManager.register("V_ACTIVITY_"..GameDef.ActivityType.ShrinePrayShop .. data.id, img_red)
        ...
        btn_free:addClickListener(function()
            ...
            local reqInfo = {
                id = data.id,
                activityType = GameDef.ActivityType.ShrinePrayShop,
            }
        ...
        btn_buy:addClickListener(function()
            ...
            ModelManager.RechargeModel:directBuy(data.price,  GameDef.StatFuncType.SFT_ShrinePrayShop, data.id,data.name,nil, data.showName1)
        end,11)
...
-- 倒计时
function ActShrineShopView:updateCountTimer()
...
function ActShrineShopView:_exit()
    TimeLib.clearCountDown(self.timer)
end


return ActShrineShopView