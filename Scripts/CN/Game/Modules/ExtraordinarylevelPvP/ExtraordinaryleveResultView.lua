local ExtraordinaryleveResultView = class("ExtraordinaryleveResultView", Window)

function ExtraordinaryleveResultView:ctor()
    self._packName = "ExtraordinarylevelPvP"
    self._compName = "ExtraordinaryleveResultView"
...
function ExtraordinaryleveResultView:_initUI()
    local root = self
    ...
    root.list_group = rootView:getChildAutoType("list_group")
	 ...
	 ExtraordinarylevelPvPModel:setNeedPlay(false)
...
function ExtraordinaryleveResultView:_initEvent()
    self._frame:addClickListener(function ()
        self:closeView()
    end)
end

function ExtraordinaryleveResultView:upGroupInfo()
    ...
    local loseCount = 2 - winCount
    self.txt_winCount:setText(winCount..":"..loseCount)
	if self._args.ackType == 2 then --����
...
function ExtraordinaryleveResultView:upGroupItem(idx, obj)
...
function ExtraordinaryleveResultView:Battle_BattleRecordData(_, param)
...
return ExtraordinaryleveResultView