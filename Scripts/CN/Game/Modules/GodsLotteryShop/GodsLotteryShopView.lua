...
local GodsLotteryShopView = class("GodsLotteryShopView",Window)

function GodsLotteryShopView:ctor()
...
function GodsLotteryShopView:_initUI()
...
function GodsLotteryShopView:_initEvent()
    self:GodsLotteryShop_refreshPanal()
end

function GodsLotteryShopView:GodsLotteryShop_refreshPanal()
    self:refreshPanal()
end

function GodsLotteryShopView:refreshPanal()
    self:setGiftList()
    self:updateCountTimer()
end

-- 设置礼包列表
function GodsLotteryShopView:setGiftList()
    local giftData = GodsLotteryShopModel:getShopData()
    self.list_gift:setItemRenderer(function(idx,obj)
        ...
        if data.price == 0 then
            ...
            RedManager.register("V_ACTIVITY_"..GameDef.ActivityType.GodsPrayShop .. data.id, img_red)
        ...
        btn_free:addClickListener(function()
            ...
            local reqInfo = {
                activityType = GameDef.ActivityType.GodsPrayShop,
                id = data.id,
            }
        ...
        btn_buy:addClickListener(function()
            ...
            -- end)
            ModelManager.RechargeModel:directBuy(data.price,  GameDef.StatFuncType.SFT_GodsPrayShop, data.id,data.name, nil,data.showName1)
        end,11)
...
-- 倒计时
function GodsLotteryShopView:updateCountTimer()
...
function GodsLotteryShopView:_exit()
