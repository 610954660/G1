--added by xhd 
--周常任务
local WeekDailyView,Super = class("WeekDailyView", Window)
local ItemCell = require "Game.UI.Global.ItemCell"
local category = GameDef.TaskCategory.Weekly
local gamePlayType = GameDef.GamePlayType.TaskWeekly
local TaskConfiger = require "Game.ConfigReaders.TaskConfiger"
local lastInterTime = 0.1
local maxInterTime = 0.5
function WeekDailyView:ctor()
	self._packName = "Task"
...
function WeekDailyView:_refresh(  )
...
function WeekDailyView:_initUI( )
    ...
    self.actBtn5 = self.view:getChildAutoType("n15")
end

function WeekDailyView:_initEvent( )
	...
	self.targetList:addEventListener(FUIEventType.Scroll,function ( ... )
		--TaskModel:setAniFlagIndex(2,true)
...
function WeekDailyView:update_Score( ... )
...
function WeekDailyView:reqGetActiAward( type,id)
	print(1,type,id)
    local params = {}
	params.id = id
	params.type = type
...
function WeekDailyView:task_update( _,gamePlayType, recordId, seq )
	self.taskData = ModelManager.TaskModel:getAllShowTask(category)
	self.targetList:setData(self.taskData)
end

--任务列表刷新
function WeekDailyView:dailyTask_update( ... )
	self:task_update()
end

--活跃值刷新
function WeekDailyView:dailyScore_update( ... )
	self:update_Score()

end

--页面退出时执行
function WeekDailyView:_exit( ... )
	for k,v in pairs(self.scheduler) do
		if self.scheduler[k] then
			Scheduler.unschedule(self.scheduler[k])
	        self.scheduler[k] = false
	...
	print(1,"WeekDailyView _exit")
end

return WeekDailyView