...
local ActNewHeroShopView,Super = class("ActNewHeroShopView", Window)


function ActNewHeroShopView:ctor()
...
function ActNewHeroShopView:_initUI()
...
function ActNewHeroShopView:_initEvent()
    self:NewHeroPray_shoprefreshPanal()
end

function ActNewHeroShopView:NewHeroPray_shoprefreshPanal()
    self:refreshPanal()
end

function ActNewHeroShopView:refreshPanal()
    self:setGiftList()
    self:updateCountTimer()
end

-- 设置礼包列表
function ActNewHeroShopView:setGiftList()
    local giftData = ActNewHeroPrayModel:getShopData()
    self.list_gift:setItemRenderer(function(idx,obj)
        ...
        if data.price == 0 and data.buyType==1 then
            ...
            RedManager.register("V_ACTIVITY_"..GameDef.ActivityType.NewHeroSummonShop .. data.id, img_red)
        ...
        btn_free:addClickListener(function()
            ...
            local reqInfo = {
                id = data.id,
                activityType = GameDef.ActivityType.NewHeroSummonShop,
            }
        ...
        btn_buy:addClickListener(function()
            ...
            if data.price == 0 and data.buyType == 2 then --取cost显示
				...
				RPCReq.Activity_HeroSummonShop_Buy({activityType=GameDef.ActivityType.NewHeroSummonShop,id = data.id,},function ( params )
                    printTable(1,"购买成功",params)
                end)
            else
                ModelManager.RechargeModel:directBuy(data.price,  GameDef.StatFuncType.SFT_NewHeroSummonShop, data.id,data.name,nil, data.showName1)
            end
        end,11)
...
-- 倒计时
function ActNewHeroShopView:updateCountTimer()
...
function ActNewHeroShopView:_exit()
