...
local LanternDrawView = class("LanternDrawView",Window)
local ItemConfiger = require "Game.ConfigReaders.ItemConfiger" --道具配置读取器

function LanternDrawView:ctor(args)
    self._packName  = "TwistEgg"
    self._compName  = "TwistEggView"
	
	self.moduleId = LanternDrawModel:getModuleId()
...
function LanternDrawView:_initUI()
	
	self.costCode = DynamicConfigData.t_LanternDraw[self.moduleId][1].costItem[1].code
	self.cost_one = DynamicConfigData.t_LanternDraw[self.moduleId][1].costItem
	self.cost_ten =  DynamicConfigData.t_LanternDraw[self.moduleId][2].costItem
	...
	if self.list_rewardShow then
		local showData = DynamicConfigData.t_LanternShow[self.moduleId].item
	...
	self:LanternDrawView_refreshPanel()
end

function LanternDrawView:_initEvent()
    ...
    RedManager.register("V_LANTERN_DRAW".."_ONE", img_redOne)
	
	local img_redTen = self.btn_tenTime:getChildAutoType("img_red")
    RedManager.register("V_LANTERN_DRAW".."_TEN", img_redTen)
	
	local skip = FileCacheManager.getStringForKey(PlayerModel.userid.."LanternDrawView_skip","0",nil,true)
	self.isSkip = skip == "1"
    ...
    self.skipEffect:addClickListener(function()
		...
		FileCacheManager.setStringForKey(PlayerModel.userid.."LanternDrawView_skip", self.isSkip and "1" or "0",nil,true)
	end)
	self.btn_shop:removeClickListener(888)
    self.btn_shop:addClickListener(function()
		ModuleUtil.openModule(ModuleId.LanternShop.id)
    end,888)
	...
	self.btn_help1:addClickListener(function()
		local config = DynamicConfigData.t_LanternPR[self.moduleId]
    ...
    self.btn_oneTime:addClickListener(function()
		if PlayerModel:isCostEnough(self.cost_one,true) then
			local reqInfo = {
				activityId    = GameDef.ActivityType.LanternDraw,
				drawType     = 1,
			}
...
function LanternDrawView:playEffect(type, reward)
...
function LanternDrawView:LanternDrawView_refreshPanel(_,params)
    local maxHistoryTimes = ModelManager.LanternDrawModel.drawCount
    ...
    for i = 1,5 do
		...
		if data1 then
			...
			local canGet = ModelManager.LanternDrawModel.drawCount >= data1.time  
			local hasGet = ModelManager.LanternDrawModel.recvRecords[i] ~= nil
			if canGet then
...
-- 倒计时
function LanternDrawView:updateCountTimer()
...
function LanternDrawView:_exit()
	self.isDrawing = false
	if self.timer then
		TimeLib.clearCountDown(self.timer)
	end
