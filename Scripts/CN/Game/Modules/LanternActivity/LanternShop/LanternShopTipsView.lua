local LanternShopTipsView = class("LanternShopTipsView",Window)
function LanternShopTipsView:ctor()
...
function LanternShopTipsView:_initUI()
...
function LanternShopTipsView:_initEvent()
	self:LanternShopTipsView_upDate()
end


function LanternShopTipsView:LanternShopTipsView_upDate()
	...
	self.btn_ok:addClickListener(function()
		if haveMoney >= (costRes.amount * buyNum) then
			local reqInfo = {
                activityId = GameDef.ActivityType.LanternShop,
				id = showData.id,
				buyCount = buyNum,
            }  
            RPCReq.Activity_GashaponShop_Buy(reqInfo,function(params)
            end)
			ViewManager.close("LanternShopTipsView")
        else
