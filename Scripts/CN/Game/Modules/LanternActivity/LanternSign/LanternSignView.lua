...
local LanternSignView = class("LanternSignView",Window)

function LanternSignView:ctor()
...
function LanternSignView:_initUI()
...
function LanternSignView:_initEvent()
	self:LanternSignView_refresh()
end

function LanternSignView:LanternSignView_refresh()
	self:updatePanel()
	self:updateActTimeShow()
end

function LanternSignView:updatePanel()
		...
		elseif self.serverData.dayIndex==index then --当天
			if self.serverData.recvRecords and self.serverData.recvRecords[index] and self.serverData.recvRecords[index].dayIndex == index then
				...
				btn_get:addClickListener(function()
					printTable(8850,">>>12333>>>")
					--领取
					local params = {}
					params.dayIndex = index
					params.activityId = LanternSignModel:getActivityId()
					printTable(8850,">>>>params>>>",params)
					params.onSuccess = function (res )
						printTable(8850,">>>res>>>",res)
					end
	...
	self.config = LanternSignModel:getShowConfig()
	self.serverData = LanternSignModel:getData()
	printTable(1,self.serverData)
	self.list_reward:setData(self.config)
end

--更新活动时间
function LanternSignView:updateActTimeShow( ... )
	if self.__timerId then
		TimeLib.clearCountDown(self.__timerId)
	end
    local actid = LanternSignModel:getActivityId( )
...
function LanternSignView:_exit()
	if self.__timerId then
		TimeLib.clearCountDown(self.__timerId)
	end
end

return LanternSignView