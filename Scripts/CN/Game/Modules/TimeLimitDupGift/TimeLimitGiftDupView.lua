--Name : TimeLimitGiftDupView.lua
--Author : generated by FairyGUI
--Date : 2020-7-10
--Desc : 
--可重复礼包UI层
local TimeLimitGiftDupView,Super = class("TimeLimitGiftDupView", Window)
local ItemCell = require "Game.UI.Global.ItemCell"
function TimeLimitGiftDupView:ctor()
	--LuaLog("TimeLimitGiftDupView ctor")
...
function TimeLimitGiftDupView:_initEvent( )
	local params = {}
...
function TimeLimitGiftDupView:_initVM( )
	local vmRoot = self
	local viewNode = self.view
	---Do not modify following code--------
	--{vmFieldsEnd}:TimeLimitGift.TimeLimitGiftDupView
	--Do not modify above code-------------
end

function TimeLimitGiftDupView:_initUI( )
	self:_initVM()
	self.data = ModelManager.TimeLimitGiftDupModel:getData()
	...
	--self.tabIndex = 1
	self.tabIndex = ModelManager.TimeLimitGiftDupModel.tabIndex or 1
	...
	if self._args.giftID then
		for i,v in ipairs(self.data) do
			if v.uid == self._args.giftID then
				self.tabIndex = i
				break
			end
		end
	...
	if not self._args.isAutoOpen then
		ModelManager.TimeLimitGiftDupModel:setNotShowRed()
	end
end
...
function TimeLimitGiftDupView:_exit( )
	for k,v in pairs(self.timerList) do
		TimeLib.clearCountDown(v)
	end
	if ModelManager.TimeLimitGiftDupModel and ModelManager.TimeLimitGiftDupModel.tabIndex~=nil then 
		ModelManager.TimeLimitGiftDupModel.tabIndex = 1 
	end
	self.timerList = {}
	self.data = false
end

function TimeLimitGiftDupView:updateTabList()
	printTable(999,"显示礼包",self.data)
	self.tabList:setNumItems(#self.data)
end

function TimeLimitGiftDupView:updateInfo()
	local data = self.data[self.tabIndex]
	if not data then  return end
	local config = DynamicConfigData.t_SurpriseGiftDupConfig[self.data[self.tabIndex].id]
	if not config then
		return
	end
...
function TimeLimitGiftDupView:updateItemList()
	local config = DynamicConfigData.t_SurpriseGiftDupConfig[self.data[self.tabIndex].id]
	if not config then
		return
	end
...
function TimeLimitGiftDupView:updateTimeCount()
	for k,v in pairs(self.timerList) do
		TimeLib.clearCountDown(v)
	end
	self.timerList = {}
...
function TimeLimitGiftDupView:updateTimeText()
...
function TimeLimitGiftDupView:onCountDown(key,time)
...
function TimeLimitGiftDupView:onCountEnd(key)
	TimeLib.clearCountDown(self.timerList[key])
	local data = self.data[key]
	if not data then return end
	print(999,"计时到了",key,self.data[key].uid,self.data[key].id)
	
	TimeLimitGiftDupModel:deleteData(self.data[key].uid)
end

function TimeLimitGiftDupView:tabListRenderer(index,obj)
	local config = DynamicConfigData.t_SurpriseGiftDupConfig[self.data[index + 1].id]
		if config then
...
function TimeLimitGiftDupView:itemListRenderer(index,obj)
	local itemCell = BindManager.bindItemCell(obj)
	local config = DynamicConfigData.t_SurpriseGiftDupConfig[self.data[self.tabIndex].id].giftItem[index + 1]
	itemCell:setData(config.code,config.amount,config.type)
end

function TimeLimitGiftDupView:onTabItemClick(index)
...
function TimeLimitGiftDupView:onBtnBuyClick()
	local info = {}
	...
	info.onYes = function()
        ...
        local config = DynamicConfigData.t_SurpriseGiftDupConfig[self.data[self.tabIndex].id]
        ModelManager.RechargeModel:directBuy(config.price, GameDef.StatFuncType.SFT_SurpriseGiftDup, giftID, config.name,self.data[self.tabIndex].uid, self.data[self.tabIndex].showName1)
	end
	Alert.show(info)
end

function TimeLimitGiftDupView:Event_updateTimeDupLimit(name)
	if tolua.isnull(self.view) then
		return
	end
	self.data = TimeLimitGiftDupModel:getData()
