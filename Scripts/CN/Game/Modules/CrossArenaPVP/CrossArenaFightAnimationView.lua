local CrossArenaFightAnimationView,Super = class("CrossArenaFightAnimationView", Window)

function CrossArenaFightAnimationView:ctor()
	self._packName = "CrossArenaPVP"
	self._compName = "CrossArenaFightAnimationView"
	self._rootDepth = LayerDepth.PopWindow
	self.__reloadPacket = true
	self.spine = false
end

function CrossArenaFightAnimationView:_initEvent()
	
end

function CrossArenaFightAnimationView:_initVM()
	local viewNode = self.view
	---Do not modify following code--------
	--{autoFields}:CrossPVP.CrossArenaFightAnimationView
	...
	--{autoFieldsEnd}:CrossPVP.CrossArenaFightAnimationView
	--Do not modify above code-------------
end

function CrossArenaFightAnimationView:_initUI()
	self:_initVM()
	self.blackbg:addClickListener(function()
		self:closeView()
		CrossArenaPVPModel:fightBegin(self._args)
	end)
	...
	self.fightLeft.heroCell1.severName:setText(CrossArenaPVPModel:getSeverName(LoginModel:getUnitServerId()))
	...
	self.fightRight.heroCell2.severName:setText(CrossArenaPVPModel:getSeverName(data.pkInfo.serverId))
...
function CrossArenaFightAnimationView:_refreshView()

end

function CrossArenaFightAnimationView:onExit_()

end

return CrossArenaFightAnimationView