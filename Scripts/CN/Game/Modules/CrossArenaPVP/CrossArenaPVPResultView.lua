local CrossArenaPVPResultView = class("CrossArenaPVPResultView", Window)

function CrossArenaPVPResultView:ctor()
    self._packName = "CrossArenaPVP"
    self._compName = "CrossArenaPVPResultView"
    self._rootDepth = LayerDepth.PopWindow
    self.isWin = self._args.addScore >= 0
	self.recordIds = self._args.battleeUuid
	self.otherPlayer = self._args.playerData
end

function CrossArenaPVPResultView:_initUI()
    local root = self
...
function CrossArenaPVPResultView:_initEvent()
    self._frame:addClickListener(function ()
        self:closeView()
    end)
end

function CrossArenaPVPResultView:upGroupInfo()
    ...
    local mySize = 1
    for k,v in pairs(self.fightData[1].groupInfo) do
        if v.playerId == PlayerModel.userid then
            mySize = v.side
            break
        end
    end
    if mySize == 1 then
        ...
        self.txt_otherName:setText(self._args.name)
    else
        self.txt_selfName:setText(self._args.name)
...
function CrossArenaPVPResultView:upGroupItem(idx, obj)
...
function CrossArenaPVPResultView:Battle_BattleRecordData(_, param)
...
return CrossArenaPVPResultView