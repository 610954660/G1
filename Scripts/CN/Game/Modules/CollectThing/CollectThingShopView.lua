...
local CollectThingShopView = class("CollectThingShopView",Window)

function CollectThingShopView:ctor()
...
function CollectThingShopView:_initUI()
    ...
    self.titleBanner    = self.view:getChildAutoType("titleBanner")
    
    self.banner:setURL("UI/TwistEgg/img_shop_banner.png")
    self.titleBanner:setURL("UI/CollectThingShop/img_shop_titlebanner.png")
    self.view:getChildAutoType("n26"):setVisible(false)
end

function CollectThingShopView:_initEvent()
    self:CollectThingShopView_refreshPanel()
end

function CollectThingShopView:CollectThingShopView_refreshPanel()
    self:refreshPanal()
end

function CollectThingShopView:refreshPanal()
...
-- 设置积分
function CollectThingShopView:setIntegral()
    local shopData = CollectThingModel:getShopData()
...
-- 设置商品列表
function CollectThingShopView:setShopList()
    local shopData = CollectThingModel:getShopData()
    self.list_goods:setItemRenderer(function(idx,obj)
        ...
        obj:addClickListener(function()
            ...
            ViewManager.open("DetectiveTrialShopTipsView", {showData = data,buyTime=data.buyTime,type = GameDef.ActivityType.CollectThingsShop})
        end,11)
...
-- 倒计时
function CollectThingShopView:updateCountTimer()
    if self.isEnd then return end
    local actData = ModelManager.ActivityModel:getActityByType(GameDef.ActivityType.CollectThingsShop)
...
function CollectThingShopView:_exit()
