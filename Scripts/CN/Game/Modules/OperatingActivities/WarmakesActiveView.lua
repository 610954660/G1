--Name : WarmakesActiveView.lua
--Author : generated by FairyGUI
--Date : 2020-5-29
--Desc : 战令系统

local WarmakesActiveView, Super = class("WarmakesActiveView", Window)
function WarmakesActiveView:ctor()
    --LuaLog("WarmakesActiveView ctor")
    self._packName = "OperatingActivities"
    self._compName = "WarmakesActiveView"
    --self._rootDepth = LayerDepth.Window
    self.viewIndexTag = GameDef.ActivityType.WarOrder
    self.starInfo = {}
    self.indexTag = 2
    self.isLeft = 0
    self.isFirst = true
...
function WarmakesActiveView:_initVM()
    local vmRoot = self
    local viewNode = self.view
    ---Do not modify following code--------
    --{vmFields}:OperatingActivities.WarmakesActiveView

    --{vmFieldsEnd}:OperatingActivities.WarmakesActiveView
    --Do not modify above code-------------
end

function WarmakesActiveView:_initUI()
    self:_initVM()
    local viewNode = self.view
...
function WarmakesActiveView:scorllItem()
    local configInfo = DynamicConfigData.t_BpActiveUp
    local id = OperatingActivitiesModel.WarmakesActiveInfo.id or 0
...
function WarmakesActiveView:showscorllItem()
    ...
    local putonglingquLv = OperatingActivitiesModel.WarmakesActiveInfo.commonRewardLv or 0
    local teshulingquLv = OperatingActivitiesModel.WarmakesActiveInfo.seniorRewardLv or 0
	...
	if OperatingActivitiesModel.WarmakesActiveInfo.isSenior == false then
    ...
    if tesustate == 1 and OperatingActivitiesModel.WarmakesActiveInfo.isSenior == true then
...
function WarmakesActiveView:showTextLv()
...
function WarmakesActiveView:showActiveTime()
    local actData = ModelManager.ActivityModel:getActityByType(GameDef.ActivityType.WarOrder)
    -- printTable(31,"活动倒计时》》》》》》》》》》》》》》",actData)
    if not actData then
        return
    end
    local id = OperatingActivitiesModel.WarmakesActiveInfo.id or 1
...
function WarmakesActiveView:showRewardList()
    local configInfo = DynamicConfigData.t_BpActiveUp
    local id = OperatingActivitiesModel.WarmakesActiveInfo.id or 0
    local type = OperatingActivitiesModel:getWarmakesActiveRealType()
    local configreward = configInfo[type]
    ...
    self.list_reward:setItemRenderer(
        function(index, obj)
            ...
            btn_get1:addClickListener(
                function(...)
                    OperatingActivitiesModel:WarOrderGetReward()
                end,
                100
            )
    ...
    local isSenior = OperatingActivitiesModel.WarmakesActiveInfo.isSenior
...
function WarmakesActiveView:showZhanlingBtn()
    for i = 1, 2, 1 do
        local btnkey = "$btn_zhanling" .. i
        local btnObj = self.view:getChildAutoType(btnkey)
        -- local txt_name=btnObj:getChildAutoType("txt_name")
        -- local img_btn=btnObj:getChildAutoType("img_btn")
        --img_btn:setURL()
        local c1 = btnObj:getController("c1")
        local isSenior = OperatingActivitiesModel.WarmakesActiveInfo.isSenior
        printTable(31, "22222222222222222222", isSenior)
        if i == 2 then
...
function WarmakesActiveView:getRewardState(lingquLv, limitLv)
    local curLv, curexp, nextExp = OperatingActivitiesModel:getWarmakesActiveLvAndExp()
...
function WarmakesActiveView:getScrollIndex()
    local configInfo = DynamicConfigData.t_BpActiveUp
    local id = OperatingActivitiesModel.WarmakesActiveInfo.id or 0
    local type = OperatingActivitiesModel:getWarmakesActiveRealType()
    local configreward = configInfo[type]
...
function WarmakesActiveView:_initEvent()
    local actData = ModelManager.ActivityModel:getActityByType(GameDef.ActivityType.WarOrder)
    ...
    self.btn_buy:addClickListener(
        function(...)
            ViewManager.open("WarmakesPayInfoView")
        end,
        100
    )
...
function WarmakesActiveView:activity_update()
    self:showActiveTime()
    self:showTextLv()
    self:showRewardList()
    self:scorllItem()
...
--事件初始化
function WarmakesActiveView:activity_WarmakesActiveupdate(...)
    self:showTextLv()
    local configInfo = DynamicConfigData.t_BpActiveUp
    local type = OperatingActivitiesModel:getWarmakesActiveRealType()
    local configreward = configInfo[type]
    ...
    if isSenior then
        ViewManager.close("WarmakesPayInfoView")
    end
...
function WarmakesActiveView:_exit(...)
    if self.calltimer then
        TimeLib.clearCountDown(self.calltimer)
    end
end

return WarmakesActiveView
