--Name : WarmakesHallowsActiveView.lua
--Author : generated by FairyGUI
--Date : 2020-5-29
--Desc : 圣器战令系统

local WarmakesHallowsActiveView, Super = class("WarmakesHallowsActiveView", Window)
function WarmakesHallowsActiveView:ctor()
    self._packName = "OperatingActivities"
    self._compName = "WarmakesActiveView"
    self.viewIndexTag = GameDef.ActivityType.HallowWarOrder
...
function WarmakesHallowsActiveView:_initVM()
    local vmRoot = self
    local viewNode = self.view
    ---Do not modify following code--------
    --{vmFields}:OperatingActivities.WarmakesHallowsActiveView

    --{vmFieldsEnd}:OperatingActivities.WarmakesHallowsActiveView
    --Do not modify above code-------------
end

function WarmakesHallowsActiveView:setActType(actType)
...
function WarmakesHallowsActiveView:_initUI()
    self:_initVM()
    local viewNode = self.view
...
function WarmakesHallowsActiveView:scorllItem()
...
function WarmakesHallowsActiveView:showscorllItem()
...
function WarmakesHallowsActiveView:showTextLv()
...
function WarmakesHallowsActiveView:showActiveTime()
...
function WarmakesHallowsActiveView:showRewardList()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesHallowsActiveView:showZhanlingBtn()
    for i = 1, 2, 1 do
...
function WarmakesHallowsActiveView:getRewardState(lingquLv, limitLv)
    local curLv, curexp, nextExp = OperatingActivitiesModel:getWarmakesElfActiveLvAndExp(self.viewIndexTag)
...
function WarmakesHallowsActiveView:getScrollIndex()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesHallowsActiveView:_initEvent()
    local actData = ModelManager.ActivityModel:getActityByType(self.viewIndexTag)
    if not actData then
       return 
    end
    ...
    self.btn_taskenter:addClickListener(
        function(...)
            ModuleUtil.openModule(ModuleId.RelicCopy.id,true)  
        end,
        100
    )
end

function WarmakesHallowsActiveView:activity_update()
    self:showActiveTime()
    self:showTextLv()
    self:showRewardList()
    self:scorllItem()
...
--事件初始化
function WarmakesHallowsActiveView:activity_WarmakesElfActiveupdate(...)
    self:showTextLv()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
...
function WarmakesHallowsActiveView:_exit(...)
    if self.calltimer then
        TimeLib.clearCountDown(self.calltimer)
    end
end

return WarmakesHallowsActiveView
