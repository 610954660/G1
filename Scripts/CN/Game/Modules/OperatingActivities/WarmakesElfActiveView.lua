--Name : WarmakesElfActiveView.lua
--Author : generated by FairyGUI
--Date : 2020-5-29
--Desc : 精灵战令系统

local WarmakesElfActiveView, Super = class("WarmakesElfActiveView", Window)
function WarmakesElfActiveView:ctor()
    --LuaLog("WarmakesElfActiveView ctor")
...
function WarmakesElfActiveView:_initVM()
    local vmRoot = self
    local viewNode = self.view
    ---Do not modify following code--------
    --{vmFields}:OperatingActivities.WarmakesElfActiveView

    --{vmFieldsEnd}:OperatingActivities.WarmakesElfActiveView
    --Do not modify above code-------------
end

function WarmakesElfActiveView:setActType(actType)
...
function WarmakesElfActiveView:_initUI()
    self:_initVM()
    local viewNode = self.view
...
function WarmakesElfActiveView:scorllItem()
...
function WarmakesElfActiveView:showscorllItem()
...
function WarmakesElfActiveView:showTextLv()
...
function WarmakesElfActiveView:showActiveTime()
...
function WarmakesElfActiveView:showRewardList()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesElfActiveView:showZhanlingBtn()
    for i = 1, 2, 1 do
...
function WarmakesElfActiveView:getRewardState(lingquLv, limitLv)
    local curLv, curexp, nextExp = OperatingActivitiesModel:getWarmakesElfActiveLvAndExp(self.viewIndexTag)
...
function WarmakesElfActiveView:getScrollIndex()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesElfActiveView:_initEvent()
    local actData = ModelManager.ActivityModel:getActityByType(self.viewIndexTag)
    if not actData then
       return 
    end
...
function WarmakesElfActiveView:activity_update()
    self:showActiveTime()
    self:showTextLv()
    self:showRewardList()
    self:scorllItem()
...
--事件初始化
function WarmakesElfActiveView:activity_WarmakesElfActiveupdate(...)
    self:showTextLv()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
...
function WarmakesElfActiveView:_exit(...)
    if self.calltimer then
        TimeLib.clearCountDown(self.calltimer)
    end
end

return WarmakesElfActiveView
