--Name : LimitedmarketView.lua
--Author : generated by FairyGUI
--Date : 2020-5-29
--Desc : 限时商城

local LimitedmarketView, Super = class("LimitedmarketView", Window)
local ItemCell = require "Game.UI.Global.ItemCell"
function LimitedmarketView:ctor()
    --LuaLog("LimitedmarketView ctor")
    self._packName = "OperatingActivities"
    self._compName = "LimitedmarketView"
...
function LimitedmarketView:_initEvent()
end

function LimitedmarketView:_initVM()
    local vmRoot = self
    local viewNode = self.view
    ---Do not modify following code--------
    --{vmFields}:OperatingActivities.LimitedmarketView
    vmRoot.txt_countdowm = viewNode:getChildAutoType("$txt_countdowm")
    --list
    vmRoot.List_reward = viewNode:getChildAutoType("$list_reward")
    --text
    --{vmFieldsEnd}:OperatingActivities.LimitedmarketView
    --Do not modify above code-------------
end

function LimitedmarketView:_initUI()
    self:_initVM()
    local dayStr = DateUtil.getOppostieDays()
    FileCacheManager.setBoolForKey("LimitedmarketView_isShow" .. dayStr, true)
    OperatingActivitiesModel:Limitedmarkered()
    self.activeInfo = ModelManager.ActivityModel:getActityByType(GameDef.ActivityType.LimitGift)
    self:showActiveTime()
    self:showRewardList()
end

function LimitedmarketView:showRewardList()
    printTable(152, "2333333333333", self.activeInfo)
    if self.activeInfo.showContent and self.activeInfo.showContent.data then
        self.rewardInfo = OperatingActivitiesModel:getLimitedmarkeConfigInfo(true)
        ...
        self.List_reward:setItemRenderer(
            function(index, obj)
                ...
                txt_desc:setText(itemMode.name)
                ...
                btn_ordinary:addClickListener(
                    function(context)
                        ...
                        if itemMode.price == 0 then
                            OperatingActivitiesModel:getActivityLimitGiftReward(itemMode.id)
                        else
                            ModelManager.RechargeModel:directBuy(
                                itemMode.price,
                                GameDef.StatFuncType.SFT_LimitGift,
                                itemMode.id,
...
function LimitedmarketView:getLingquState(id, time)
    local lingqu = false
    local lingquList = OperatingActivitiesModel.LimitedmarkeActiveInfo
...
function LimitedmarketView:getLingquCount(id)
    local count = 0
    local lingquList = OperatingActivitiesModel.LimitedmarkeActiveInfo
    if not lingquList then
        return count
...
function LimitedmarketView:showActiveTime()
...
--事件初始化
function LimitedmarketView:activity_LimitedmarketActiveUpdate(...)
    self.activeInfo = ModelManager.ActivityModel:getActityByType(GameDef.ActivityType.LimitGift)
...
function LimitedmarketView:_exit(...)
    if self.calltimer then
        TimeLib.clearCountDown(self.calltimer)
    end
end

return LimitedmarketView
