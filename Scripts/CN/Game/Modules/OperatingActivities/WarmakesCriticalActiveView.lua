--Name : WarmakesCriticalActiveView.lua
--Author : generated by FairyGUI
--Date : 2020-5-29
--Desc : 临界战令系统

local WarmakesCriticalActiveView, Super = class("WarmakesCriticalActiveView", Window)
function WarmakesCriticalActiveView:ctor()
    self._packName = "OperatingActivities"
    self._compName = "WarmakesActiveView"
    self.viewIndexTag = GameDef.ActivityType.BoundaryWarOrder
...
function WarmakesCriticalActiveView:_initVM()
    local vmRoot = self
    local viewNode = self.view
    ---Do not modify following code--------
    --{vmFields}:OperatingActivities.WarmakesCriticalActiveView

    --{vmFieldsEnd}:OperatingActivities.WarmakesCriticalActiveView
    --Do not modify above code-------------
end

function WarmakesCriticalActiveView:setActType(actType)
...
function WarmakesCriticalActiveView:_initUI()
    self:_initVM()
    local viewNode = self.view
...
function WarmakesCriticalActiveView:scorllItem()
...
function WarmakesCriticalActiveView:showscorllItem()
...
function WarmakesCriticalActiveView:showTextLv()
...
function WarmakesCriticalActiveView:showActiveTime()
    ...
    local lastTime =TimeLib.nextMonthBeginTime()-ServerTimeModel:getServerTime() --(id * 30 * 24 * 60 * 60) - (curSerevrTime - starTime)
...
function WarmakesCriticalActiveView:showRewardList()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesCriticalActiveView:showZhanlingBtn()
    for i = 1, 2, 1 do
...
function WarmakesCriticalActiveView:getRewardState(lingquLv, limitLv)
    local curLv, curexp, nextExp = OperatingActivitiesModel:getWarmakesElfActiveLvAndExp(self.viewIndexTag)
...
function WarmakesCriticalActiveView:getScrollIndex()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesCriticalActiveView:_initEvent()
    local actData = ModelManager.ActivityModel:getActityByType(self.viewIndexTag)
    if not actData then
       return 
    end
...
function WarmakesCriticalActiveView:activity_update()
    self:showActiveTime()
    self:showTextLv()
    self:showRewardList()
    self:scorllItem()
...
--事件初始化
function WarmakesCriticalActiveView:activity_WarmakesElfActiveupdate(...)
    self:showTextLv()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
...
function WarmakesCriticalActiveView:_exit(...)
    if self.calltimer then
        TimeLib.clearCountDown(self.calltimer)
    end
end

return WarmakesCriticalActiveView
