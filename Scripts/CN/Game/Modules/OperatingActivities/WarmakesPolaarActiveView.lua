--Name : WarmakesPolaarActiveView.lua
--Author : generated by FairyGUI
--Date : 2020-5-29
--Desc : 极地战令系统

local WarmakesPolaarActiveView, Super = class("WarmakesPolaarActiveView", Window)
function WarmakesPolaarActiveView:ctor()
    self._packName = "OperatingActivities"
    self._compName = "WarmakesActiveView"
    self.viewIndexTag = GameDef.ActivityType.EndlessRoadWarOrder
...
function WarmakesPolaarActiveView:_initVM()
    local vmRoot = self
    local viewNode = self.view
    ---Do not modify following code--------
    --{vmFields}:OperatingActivities.WarmakesPolaarActiveView

    --{vmFieldsEnd}:OperatingActivities.WarmakesPolaarActiveView
    --Do not modify above code-------------
end

function WarmakesPolaarActiveView:setActType(actType)
...
function WarmakesPolaarActiveView:_initUI()
    self:_initVM()
    local viewNode = self.view
...
function WarmakesPolaarActiveView:scorllItem()
...
function WarmakesPolaarActiveView:showscorllItem()
...
function WarmakesPolaarActiveView:showTextLv()
...
function WarmakesPolaarActiveView:showActiveTime()
...
function WarmakesPolaarActiveView:showRewardList()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesPolaarActiveView:showZhanlingBtn()
    for i = 1, 2, 1 do
...
function WarmakesPolaarActiveView:getRewardState(lingquLv, limitLv)
    local curLv, curexp, nextExp = OperatingActivitiesModel:getWarmakesElfActiveLvAndExp(self.viewIndexTag)
...
function WarmakesPolaarActiveView:getScrollIndex()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesPolaarActiveView:_initEvent()
    local actData = ModelManager.ActivityModel:getActityByType(self.viewIndexTag)
    if not actData then
       return 
    end
    ...
    self.btn_taskenter:addClickListener(
        function(...)
            ModuleUtil.openModule(ModuleId.EndlessRoad.id,true)  
        end,
        100
    )
end

function WarmakesPolaarActiveView:activity_update()
    self:showActiveTime()
    self:showTextLv()
    self:showRewardList()
    self:scorllItem()
...
--事件初始化
function WarmakesPolaarActiveView:activity_WarmakesElfActiveupdate(...)
    self:showTextLv()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
...
function WarmakesPolaarActiveView:_exit(...)
    if self.calltimer then
        TimeLib.clearCountDown(self.calltimer)
    end
end

return WarmakesPolaarActiveView
