--Name : WarmakesSmallElfActiveView.lua
--Author : generated by FairyGUI
--Date : 2020-5-29
--Desc : 小额精灵战令系统

local WarmakesSmallElfActiveView, Super = class("WarmakesSmallElfActiveView", Window)
function WarmakesSmallElfActiveView:ctor()
    --LuaLog("WarmakesSmallElfActiveView ctor")
...
function WarmakesSmallElfActiveView:_initVM()
    local vmRoot = self
    local viewNode = self.view
    ---Do not modify following code--------
    --{vmFields}:OperatingActivities.WarmakesSmallElfActiveView

    --{vmFieldsEnd}:OperatingActivities.WarmakesSmallElfActiveView
    --Do not modify above code-------------
end

function WarmakesSmallElfActiveView:setActType(actType)
...
function WarmakesSmallElfActiveView:_initUI()
    self:_initVM()
    local viewNode = self.view
...
function WarmakesSmallElfActiveView:scorllItem()
...
function WarmakesSmallElfActiveView:showscorllItem()
...
function WarmakesSmallElfActiveView:showTextLv()
...
function WarmakesSmallElfActiveView:showActiveTime()
    ...
    local lastTime = (id * 7 * 24 * 60 * 60) - (curSerevrTime - starTime)
...
function WarmakesSmallElfActiveView:showRewardList()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesSmallElfActiveView:showZhanlingBtn()
    for i = 1, 2, 1 do
...
function WarmakesSmallElfActiveView:getRewardState(lingquLv, limitLv)
    local curLv, curexp, nextExp = OperatingActivitiesModel:getWarmakesElfActiveLvAndExp(self.viewIndexTag)
...
function WarmakesSmallElfActiveView:getScrollIndex()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
    if not configreward then
...
function WarmakesSmallElfActiveView:_initEvent()
    local actData = ModelManager.ActivityModel:getActityByType(self.viewIndexTag)
    if not actData then
       return 
    end
...
function WarmakesSmallElfActiveView:activity_update()
    self:showActiveTime()
    self:showTextLv()
    self:showRewardList()
    self:scorllItem()
...
--事件初始化
function WarmakesSmallElfActiveView:activity_WarmakesElfActiveupdate(...)
    self:showTextLv()
    local configInfo = DynamicConfigData.t_BpJlActiveUp
    local type = OperatingActivitiesModel:getWarmakesElfActiveRealType(self.viewIndexTag)
    local configreward = configInfo[type]
...
function WarmakesSmallElfActiveView:_exit(...)
    if self.calltimer then
        TimeLib.clearCountDown(self.calltimer)
    end
end

return WarmakesSmallElfActiveView
