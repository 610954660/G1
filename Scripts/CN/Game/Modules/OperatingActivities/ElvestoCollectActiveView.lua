--Name : ElvestoCollectActiveView.lua
--Author : generated by FairyGUI
--Date : 2020-5-29
--Desc : 精灵收集

local ElvestoCollectActiveView, Super = class("ElvestoCollectActiveView", Window)
function ElvestoCollectActiveView:ctor()
    LuaLog("ElvestoCollectActiveView ctor")
    self._packName = "OperatingActivities"
    self._compName = "ElvestoCollectActiveView"
    --self._rootDepth = LayerDepth.Window
    self.viewIndexTag = GameDef.ActivityType.ElfCollection
    self.starInfo = false
    self.bannerUrl = false
    self.calltimer = false
end

function ElvestoCollectActiveView:_initEvent()
end

function ElvestoCollectActiveView:_initVM()
    local vmRoot = self
    local viewNode = self.view
    ---Do not modify following code--------
    --{vmFields}:OperatingActivities.ElvestoCollectActiveView
    ...
    --loader
    --{vmFieldsEnd}:OperatingActivities.ElvestoCollectActiveView
    --Do not modify above code-------------
end

function ElvestoCollectActiveView:setActType(_args)
end

function ElvestoCollectActiveView:_initUI()
    self:_initVM()
    local dayStr = DateUtil.getOppostieDays()
    FileCacheManager.setBoolForKey("ElvestoCollectActiveView_isShow" .. dayStr, true)
...
function ElvestoCollectActiveView:showCountTime()
    local actData = ModelManager.ActivityModel:getActityByType(self.viewIndexTag)
    if not actData then
        return
    end
...
function ElvestoCollectActiveView:showView()
    self:showCountTime()
        ...
        function(index, obj)
            ...
            btn:addClickListener(
                function()
                    ...
                    OperatingActivitiesModel:ElvestoCollectRecieveReward(actData.id, taskId)
                end,
                100
            )
...
function ElvestoCollectActiveView:activity_ElvestoCollectUpdate(...)
    self.starInfo = OperatingActivitiesModel:getElvestoCollectActiveMapInfo(self.viewIndexTag, true)
    self.list_item:setNumItems(#self.starInfo)
end

function ElvestoCollectActiveView:_exit()
    if self.calltimer then
        TimeLib.clearCountDown(self.calltimer)
    end
end

return ElvestoCollectActiveView
