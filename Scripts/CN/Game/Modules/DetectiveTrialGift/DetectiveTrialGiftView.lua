...
local DetectiveTrialGiftView = class("DetectiveTrialGiftView",Window)

function DetectiveTrialGiftView:ctor()
...
function DetectiveTrialGiftView:_initUI()
...
function DetectiveTrialGiftView:_initEvent()
    self:DetectiveTrialGift_refreshPanal()
end

function DetectiveTrialGiftView:DetectiveTrialGift_refreshPanal()
    self:refreshPanal()
end

function DetectiveTrialGiftView:refreshPanal()
    self:setGiftList()
    self:updateCountTimer()
end

-- 设置礼包列表
function DetectiveTrialGiftView:setGiftList()
    local giftData = DetectiveTrialGiftModel:getShopData()
    self.list_gift:setItemRenderer(function(idx,obj)
        ...
        if data.price == 0 then
            ...
            RedManager.register("V_ACTIVITY_"..GameDef.ActivityType.HeroTrialGift .. data.id, img_red)
        ...
        btn_free:addClickListener(function()
            ...
            local reqInfo = {
                activityId = GameDef.ActivityType.HeroTrialGift,
                id = data.id,
            }
        ...
        btn_buy:addClickListener(function()
            ...
            -- end)
            ModelManager.RechargeModel:directBuy(data.price,  GameDef.StatFuncType.SFT_HeroTrialGift, data.id,data.name, nil,data.showName1)
        end,11)
...
-- 倒计时
function DetectiveTrialGiftView:updateCountTimer()
...
function DetectiveTrialGiftView:_exit()
